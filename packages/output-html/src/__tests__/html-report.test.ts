...
describe("HtmlReportGenerator", () => {
  ...
  it("should include AspidaSec branding in header", () => {
    const report = makeReport();
    const html = generator.generateReport(report, makeConfig());

    expect(html).toContain("AspidaSec");
  ...
  it("should render footer with AspidaSec attribution", () => {
    const report = makeReport();
    const html = generator.generateReport(
      report,
      makeConfig({ locale: "id" }),
    ...
    expect(html).toContain("Dibuat oleh AspidaSec");
  });
...
describe("templates", () => {
  ...
  it("footerTemplate should include bilingual attribution", () => {
    ...
    expect(enFooter).toContain("Generated by AspidaSec");

    const idFooter = footerTemplate("id", "2026-01-15T10:00:00Z");
    expect(idFooter).toContain("Dibuat oleh AspidaSec");
  });
