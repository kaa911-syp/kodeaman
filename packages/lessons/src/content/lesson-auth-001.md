...
Autentikasi adalah cara aplikasi memverifikasi siapa pengguna itu. Autentikasi yang lemah adalah vektor serangan utama — broken auth secara konsisten muncul di OWASP Top 10.

### Kerentanan Umum

```php
// RENTAN — Validasi password lemah di Laravel
$request->validate([
    'password' => 'required|min:4',  // Terlalu pendek, tanpa kompleksitas
]);

// RENTAN — Tidak ada rate limiting di login
Route::post('/login', [AuthController::class, 'login']);
```

```javascript
// RENTAN — Menyimpan password dalam plain text
const user = { email, password: req.body.password };
await db.collection('users').insertOne(user);

// RENTAN — Konfigurasi JWT yang lemah
const token = jwt.sign(payload, 'secret', { algorithm: 'none' });
```
...
```php
// AMAN — Aturan password kuat di Laravel
$request->validate([
    'password' => ['required', 'min:8', 'confirmed',
        Password::min(8)->mixedCase()->numbers()->symbols()],
]);

// AMAN — Rate limiting dengan Laravel
Route::post('/login', [AuthController::class, 'login'])
    ->middleware('throttle:5,1');  // 5 percobaan per menit

// AMAN — Gunakan hashing bawaan Laravel
$user->password = Hash::make($request->password);
```
...
// AMAN — Konfigurasi JWT yang kuat
const token = jwt.sign(payload, process.env.JWT_SECRET, {
  algorithm: 'HS256',
  expiresIn: '1h',
});

// AMAN — Rate limiting dengan express-rate-limit
import rateLimit from 'express-rate-limit';
const loginLimiter = rateLimit({ windowMs: 60000, max: 5 });
app.post('/login', loginLimiter, loginHandler);
```
...
- Selalu hash password dengan bcrypt (cost factor 10+)
- Implementasikan rate limiting di endpoint autentikasi
- Gunakan algoritma JWT yang kuat (HS256/RS256), jangan pernah `none`
- Atur waktu kedaluwarsa sesi/token yang sesuai
- Implementasikan autentikasi multi-faktor untuk aksi sensitif
