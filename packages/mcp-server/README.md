...
AspidaSec MCP Server exposes AspidaSec's security scanning workflow through the Model Context Protocol (MCP), so AI coding assistants can run scans, inspect findings, produce reports, and request remediation guidance from inside a development session.

## Overview

The server runs over MCP stdio and registers AspidaSec tools for project-level application security checks. It connects assistants such as Claude Code, Cursor, and Windsurf to the same scanner adapters and report converters used by the AspidaSec CLI.

Use it to:

- Run general AspidaSec security scans against a local project directory.
...
```bash
npm install -g @aspidasec/mcp-server
```

Or run it without a global install:

```bash
npx @aspidasec/mcp-server
```

For local monorepo development, build the package first:

```bash
pnpm --filter @aspidasec/mcp-server build
node packages/mcp-server/dist/index.js
```

The published binary is named `aspidasec-mcp`.

## Available Tools

| Tool | Purpose | Typical use |
| --- | --- | --- |
| `scan` | Runs a AspidaSec scan for a project directory. | Broad security review before opening a PR. |
...
| `list-scanners` | Lists available AspidaSec scanner adapters. | Discovering which scanners can run in the current environment. |
...
```bash
claude mcp add aspidasec -- aspidasec-mcp
```
...
```text
Use AspidaSec to run preflight checks for this repository.
```
...
```text
Use AspidaSec to scan C:/Users/user2/work/os/my-app and summarize high severity findings.
```
...
```text
Use AspidaSec to run an OWASP Top 10 scan for the current project and show evidence for each finding.
```
...
```text
Use AspidaSec to explain this finding and suggest a safe fix: SQL injection in src/routes/users.ts.
```
...
```text
Use AspidaSec to convert the latest findings to SARIF for CI upload.
```
