...
SARIF (Static Analysis Results Interchange Format) is a JSON format for static-analysis results. AspidaSec can emit SARIF v2.1.0 so findings can be reviewed in IDEs, uploaded to GitHub Code Scanning, or consumed by other SARIF-compatible security tools.
...
For AspidaSec, SARIF is the interchange format between scan findings and downstream security review tools. The normalized AspidaSec findings remain the source data; SARIF is the portable representation for IDEs and CI systems.

## Creating SARIF with the MCP server

AspidaSec exposes SARIF in two MCP paths:

1. Run `aspidasec_scan` or `aspidasec_owasp_scan` with `format: "sarif"`.
2. Convert an existing findings array with `aspidasec_convert_sarif`.
...
Use `aspidasec_convert_sarif` when the assistant already has JSON scan findings and you want to convert only the findings array:
...
The tool returns raw SARIF JSON. Save that JSON to a file such as `aspidasec-results.sarif` or `aspidasec-results.sarif.json` before loading it into an IDE or uploading it to CI.
...
2. Generate a AspidaSec SARIF file using either `aspidasec_scan` with `format: "sarif"` or `aspidasec_convert_sarif`.
3. Save the result as `aspidasec-results.sarif` in or near the scanned workspace.
...
```text
Scan this workspace with AspidaSec, convert the findings to SARIF, and save the raw SARIF JSON as aspidasec-results.sarif.
```

The assistant should:

1. Call `aspidasec_scan` with the workspace absolute path.
2. Extract the `findings` array from the JSON response.
3. Call `aspidasec_convert_sarif` with the stringified findings array.
4. Write only the returned SARIF JSON to `aspidasec-results.sarif`.
...
1. Generate `aspidasec-results.sarif` from AspidaSec.
...
2. Run AspidaSec and produce SARIF.
...
| --- | --- |
| You want one assistant call to scan and export. | `aspidasec_scan` with `format: "sarif"`. |
| You already have JSON findings from a scan. | `aspidasec_convert_sarif` with the stringified findings array. |
| You need OWASP category evidence and SARIF output. | `aspidasec_owasp_scan` with `format: "sarif"`. |
...
- Regenerate the file using `format: "sarif"` or `aspidasec_convert_sarif`.
...
- Extract the `findings` field from `aspidasec_scan` output before calling `aspidasec_convert_sarif`.
